...
- Secrets/config: AWS Secrets Manager, injected into App Runner at runtime by ARN.
- Logs: stdout/stderr to CloudWatch Logs. File logging is disabled in `appsettings.Production.json`.

## Required AWS configuration

Non-sensitive runtime configuration is set directly in the API service:
...
Sensitive values are stored in AWS Secrets Manager and injected into App Runner by secret ARN:

```text
ConnectionStrings__ToDoAppConnectionString
ConnectionStrings__ToDoAppAuthConnectionString
Jwt__Key
```
...
5. Build and push the Docker image to the ECR repository created by Terraform.
6. Enable `create_app_runner_service` in Terraform, provide the secret ARNs and apply again to create App Runner.
7. Configure the final App Runner URL as `api_public_url` and apply once more so JWT issuer/audience match the API URL.
8. Apply the EF migrations against the clean RDS databases.
9. Update the Amplify frontend API base URL to the new AWS API URL.
10. After validation, cut traffic from Azure to AWS and decommission the Azure resources.
